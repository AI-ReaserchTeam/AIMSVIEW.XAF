...
using AIMSVIEW.XAF.WebApi.JWT;
using DevExpress.ExpressApp.Security.Authentication.ClientServer;
using AIMSVIEW.XAF.WebApi.Core;
using DevExpress.ExpressApp;
using DevExpress.ExpressApp.ApplicationBuilder;
using AIMSVIEW.XAF.Module.BusinessObjects;

namespace AIMSVIEW.XAF.WebApi;
...
public class Startup
{
	...
	public void ConfigureServices(IServiceCollection services)
	{
		...
		services.AddXafWebApi(builder =>
		{
			...
			builder.ConfigureOptions(options =>
			{
				...
				options.BusinessObject<Apartment>();
				options.BusinessObject<Rent>();
				options.BusinessObject<Tenant>();
				options.BusinessObject<Property>();
				options.BusinessObject<Payment>();
			});
			...
			builder.Modules
				...
				.AddValidation()
				.Add<AIMSVIEW.XAF.Module.XAFModule>();
			...
			builder.Security
				.UseIntegratedMode(options =>
				{
					...
					options.UserType = typeof(AIMSVIEW.XAF.Module.BusinessObjects.ApplicationUser);
					// ApplicationUserLoginInfo is only necessary for applications that use the ApplicationUser user type.
					// If you use PermissionPolicyUser or a custom user type, comment out the following line:
					options.UserLoginInfoType = typeof(AIMSVIEW.XAF.Module.BusinessObjects.ApplicationUserLoginInfo);
			...
			{
				application.ApplicationName = "SetupApplication.AIMSVIEW.XAF";
		...
		services.AddSwaggerGen(c =>
		{
			...
			{
				Title = "AIMSVIEW.XAF API",
				Version = "v1",
				Description = @"Use AddXafWebApi(options) in the AIMSVIEW.XAF.WebApi\Startup.cs file to make Business Objects available in the Web API."
			});
				...
				Flows = new OpenApiOAuthFlows()
				{
					AuthorizationCode = new OpenApiOAuthFlow()
					{
						...
						Scopes = new Dictionary<string, string> {
                            ...
                            { @"[Enter the scope name in the AIMSVIEW.XAF.WebApi\Startup.cs file]", @"[Enter the scope description in the AIMSVIEW.XAF.WebApi\Startup.cs file]" }
						}
					}
				}
			});
	...
	public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
	{
		if (env.IsDevelopment())
		{
			...
			app.UseSwaggerUI(c =>
			{
				c.SwaggerEndpoint("/swagger/v1/swagger.json", "AIMSVIEW.XAF WebApi v1");
